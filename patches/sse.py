...
class SseServerTransport:
    ...
    async def connect_sse(self, scope: Scope, receive: Receive, send: Send):  # pragma: no cover
        ...
        read_stream_writer, read_stream = anyio.create_memory_object_stream(32)
        write_stream, write_stream_reader = anyio.create_memory_object_stream(32)
        ...
        async with anyio.create_task_group() as tg:
            ...
            async def response_wrapper(scope: Scope, receive: Receive, send: Send):
                ...
                logger.info(f"Client session disconnected and cleaned up: {session_id}")
    ...
    async def handle_post_message(self, scope: Scope, receive: Receive, send: Send) -> None:  # pragma: no cover
        ...
        try:
            ...
            try:
                ...
                logger.warning(f"Session {session_id} disconnected while sending validation error")
            return
        ...
        try:
            ...
            logger.warning(f"Session {session_id} disconnected before message could be delivered")
